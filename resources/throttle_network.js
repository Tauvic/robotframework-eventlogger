...
async function setCPUSlowdown(rate, page, logger) {

    logger("Setting cpu speed to: " + rate + "x slowdown")

    // create a new CDP session (only supports Chrominum based browsers)
    const client = await page.context().newCDPSession(page);

    await client.send("Emulation.setCPUThrottlingRate", {
        rate: Number(rate), // 1 = no throttle, 2 = 2x slowdown, etc
    });
}
...
setCPUSlowdown.rfdoc = `
Slowdown the CPU to the given rate.
.`
...
exports.setCPUSlowdown = setCPUSlowdown;